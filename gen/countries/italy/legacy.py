...
``missing_coordinates`` unless an explicit review decision records that the
station is handled by another provider.
"""
...
HANDLED_OUTSIDE_PROVIDER_DECISIONS = {
    "handled_by_france_with_rfi_fallback",
}
OPERATOR_NAMES = {
    "fn": "Ferrovienord",
...
def find_rfi_station_page() -> Path:
    """Use the canonical RFI page or the newest saved snapshot."""
...
def handled_outside_provider_review(
    config: dict,
    operator: str,
    station_id: str,
) -> dict | None:
    """Return an explicit decision that keeps a source ID out of this dataset."""
    for review in config.get("reviews", []):
        if (
            str(review.get("operator", "")) == operator
            and str(review.get("id", "")) == station_id
            and review.get("decision") in HANDLED_OUTSIDE_PROVIDER_DECISIONS
        ):
            return review
    return None
...
def rebuild(
    ...
    *,
    ...
    # Keep reviewed order, coordinates and metadata. Provider data is
            ...
            audit.append(
                {
                    ...
                    "reviewed_name": str(outside_provider.get("reviewed_name", "")),
                    "status": "handled_by_other_provider",
                    "details": str(
                        outside_provider.get("details")
                        or "handled by another provider"
                    ),
                }
            )
            continue
        ...
        audit.append(
            {
                ...
                "details": (
                    "listed in overrides/exclusions/italy.json"
                    if is_excluded
